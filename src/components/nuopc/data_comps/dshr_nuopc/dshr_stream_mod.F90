module dshr_stream_mod
  ...
  public :: dshr_stream_getDomainInfo     ! return the stream's domain info data
  public :: dshr_stream_getFile           ! acquire file, return name of file to open
  ...
  subroutine dshr_stream_init(strm,infoFile,yearFirst,yearLast,yearAlign,taxMode,rc)
    ...
    if (n==0) then
       ...
       if (n==0) then
          rCode = 1
          write(s_logunit,F00) "ERROR: no input field names"
          call dshr_stream_abort(subName//"ERROR: no time variable name")
       else
          ...
          if(len_trim(substr) <= len(strm%domTVarName)) then
             ...
             call dshr_stream_abort(subName//"ERROR: string too long for variable name")
          endif
       endif
       ...
       if (n==0) then
          rCode = 1
          write(s_logunit,F00) "ERROR: no input field names"
          call dshr_stream_abort(subName//"ERROR: no lon variable name")
       else
          call shr_string_listGetName (fldListFile,n,substr,rc)
       ...
       if (n==0) then
          rCode = 1
          write(s_logunit,F00) "ERROR: no input field names"
          call dshr_stream_abort(subName//"ERROR: no lat variable name")
       else
          call shr_string_listGetName (fldListFile,n,substr,rc)
       ...
       if (n==0) then
          !         rCode = 1
          !         write(s_logunit,F00) "ERROR: no input field names"
          !         call dshr_stream_abort(subName//"ERROR: no area variable name")
       ...
       if (n==0) then
          !         rCode = 1
          !         write(s_logunit,F00) "ERROR: no input field names"
          !         call dshr_stream_abort(subName//"ERROR: no mask variable name")
...
999 continue
  ...
  subroutine dshr_stream_readTCoord(strm,k,rc)
    ...
    call dshr_stream_getFile(strm%filePath, strm%file(k)%name, fileName)
    ...
    call dshr_stream_verifyTCoord(strm,k,lrc) ! check new t-coord data
  ...
  !===============================================================================
  subroutine dshr_stream_verifyTCoord(strm,k,rc)
  ...
  end subroutine dshr_stream_verifyTCoord
  ...
  subroutine dshr_stream_getCalendar(strm,k,calendar)
    ...
    call dshr_stream_getFile(strm%filePath,strmfile,fileName)
    rCode = nf90_open(fileName,nf90_nowrite,fid)
  ...
  !===============================================================================
  subroutine dshr_stream_getDomainInfo(strm, filePath, fileName, timeName, &
  ...
  end subroutine dshr_stream_getDomainInfo

  !===============================================================================
  subroutine dshr_stream_getFile(filePath,fileName,localFile,rc)
  ...
  end subroutine dshr_stream_getFile
  ...
  subroutine dshr_stream_restWrite(strm,fileName,caseName,caseDesc,nstrms,rc)
    ...
    if (present(nstrms)) then
       if (size(strm) < nstrms) then
          ...
          call dshr_stream_abort(subname//": ERROR: nstrms too large for strm")
       endif
       nStreams = nstrms
    else
       nStreams = size(strm)
